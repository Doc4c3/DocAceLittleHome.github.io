...
class SM4 {
    ...
    private static $SboxTable = [ 0xD6, 0x90, 0xE9, 0xFE, 0xCC, 0xE1, 0x3D, 0xB7, 0x16, 0xB6, 0x14, 0xC2, 0x28, 0xFB, 0x2C, 0x05,
    ...
    private function cryptBlock($block, $mode) {
        ...
        for ($i = 0; $i < 32; $i++) {
            $roundKey = ($mode == self::ENCRYPT) ? $this->sk[$i] : $this->sk[31 - $i];
...
try {
    ...
    $plaintext = $sm4->decrypt($cipher_raw);
    echo "解密结果: " . $plaintext . "\n";
} catch (Exception $e) {
    echo $e->getMessage();
}
